...
수정: forced_accident / forced_event를 config에서 읽어서 simulate_day에 전달
"""
import json, os
from datetime import datetime
from src.config import CITY, EQUIPMENT_AGE, KMA_API_KEY, GITHUB_TOKEN
...
def main():
    ...
    forced_acc    = cfg.get("forced_accident",  "none")   # [수정] 읽기
    forced_event  = cfg.get("special_event",    "auto")   # [수정] 읽기
    ...
    rows = simulate_day(
        ...
        forced_accident   = forced_acc,    # [수정] 전달
        forced_event      = forced_event,  # [수정] 전달
    )
    print(f"  완료: {len(rows)}행 생성")

    # GitHub push (누적)
    print("\n[3/3] GitHub push 중 (기존 데이터 누적)...")
    ok = push_simulation_results(
        csv_path=output_csv,
        token=GITHUB_TOKEN,
        repo=DATA_REPO,
    )
