"""
simulator.py — 시뮬레이션 실행 함수
수정: recent_accidents 키 오류 수정 (accident_type으로 정상 추적)
"""
...
def get_occupancy(slot, day_type):
    ...
    base = {"night":0.00,"commute":0.40,"work_am":0.85,
            "lunch":0.50,"work_pm":0.90,"evening":0.20}
    return round(max(0.0, min(1.0, base.get(slot,0.5) + random.uniform(-0.08,0.08))),2)

def get_special_event(slot, day_type, forced_event="auto"):
    """
    forced_event: "auto" → 자동, "none"/"overtime"/... → 강제 지정
    """
    if forced_event != "auto":
        return forced_event
    if day_type in ["weekend","holiday"] or slot == "night":
        return "none"
...
def save_to_csv(rows, filepath):
    ...
    print(f"CSV 저장 완료 → {filepath} ({len(rows)}행)")

def simulate_day(date, weather_data, equipment_age_years=EQUIPMENT_AGE,
                 output_csv="panel_simulation.csv",
                 forced_accident="none", forced_event="auto"):
    """
    forced_accident: "none" or 사고유형 문자열 → 해당 사고 확률 0.99 강제 설정
    forced_event:    "auto" or 이벤트 문자열
    """
    ...
    if forced_accident != "none":
        print(f" ⚙️ 강제 사고: {forced_accident}")
    ...
    for hour in range(24):
        ...
        ctx = {
            ...
            "special_event":       get_special_event(slot, day_type, forced_event),
            ...
            "forced_accident":     forced_accident,   # 추가
        }
        ...
        # [수정] accident_type 키로 정상 추적
        acc_type = result.get("accident_type", "none")
        if acc_type != "none":
            recent_accidents.append(acc_type)
        ...
        print(f"  [{dt.strftime('%H:%M')}] "
              ...
              f"[{result['panel_status']:6}] | 사고: {acc_type}")
